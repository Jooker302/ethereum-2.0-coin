import Image from "next/image";
import { faTelegram, faXTwitter } from "@fortawesome/free-brands-svg-icons";
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";

export default function Home() {
  return (
    <>
      <div
        id="socialbar"
        class="flex justify-center bg-[#48E8ED] py-1 px-4 items-center"
      >
        <div className="flex justify-between">
          <a
            className="m-2"
            href=""
          >
            <FontAwesomeIcon
              icon={faXTwitter}
              size="lg"
              className="text-black hover:text-[#24038B]"
              style={{ width: "24px", height: "24px" }}
            />
          </a>
          <a className="m-2" href="">
            <FontAwesomeIcon
              icon={faTelegram}
              size="lg"
              className="text-black hover:text-[#24038B]"
              style={{ width: "24px", height: "24px" }}
            />
          </a>
        </div>
      </div>

      

      <div id="intro" className="flex justify-between p-4 flex-wrap">
        <div className="md:w-1/2">
          <img className="md:w-96 md:h-96 md:ml-28" src="/main.jpeg" />
        </div>
        <div className="md:w-1/2 px-4 py-2">
          <h2 className="text-[#48E8ED] text-5xl font-bold underline mt-4 md:mt-0">
          Ethereum 2.0
          </h2>
          <p className="p-4 pt-8 text-xl">
          Step into a world of decentralized innovation with Ethereum 2.0, where blockchain technology meets a new era of scalability, sustainability, and inclusivity. ETH 2.0 is not just an upgrade; it's a transformational shift towards a more efficient, eco-friendly, and accessible blockchain ecosystem. This cutting-edge upgrade redefines the potential of decentralized applications (dApps), smart contracts, and beyond, all while minimizing its carbon footprint and maximizing its impact on global scalability.
          </p>
          <div className="flex justify-center my-3">
            <a
              href="#about"
              className="px-4 py-2 cursor-pointer bg-[#48E8ED] text-black hover:bg-[#24038B] rounded-lg"
            >
              About Us
            </a>
          </div>
        </div>
      </div>

      <div id="boxes" class="md:mt-10 mt-6 flex justify-center py-8 flex-wrap">
        <div class="w-full md:w-1/4 my-6 md:my-0 bg-[#48E8ED] shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-black">Token Name</h2>
            <p class="font-semibold text-black">
            Ethereum (ETH)
            </p>
          </div>
        </div>

        <div class="w-full md:w-1/4 bg-[#48E8ED] my-6 md:my-0 shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-black">
            Total Supply
            </h2>
            <p class="font-semibold text-black">
            115,112,005 ETH (as of September 2021)
            </p>
          </div>
        </div>

        <div class="w-full md:w-1/4 bg-[#48E8ED] my-6 md:my-0 shadow-md rounded-lg md:mx-4">
          <div class="relative">
            <img
              src="/main.jpeg"
              alt="Image 2"
              class="absolute top-0 w-12 h-12 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rounded-full"
            />
          </div>
          <div class="p-4">
            <h2 class="text-xl font-bold mb-2 mt-4 text-black">Max Supply</h2>
            <p class=" font-semibold text-black">
            No fixed supply cap (infinite issuance, with a capped annual issuance under ETH 2.0)
            </p>
          </div>
        </div>
      </div>

      <div
        id="banner"
        className="py-10 px-10 md:pb-14 flex items-center justify-center"
      >
        <img
          src="/cover.jpeg"
          alt="Pulsating Image"
          className="animated-image"
        />
      </div>

      <div id="about" className="p-6 flex justify-center flex-wrap">
        <div className="bg-[#48E8ED] rounded-lg flex justify-center flex-col items-center shadow-md m-2 p-4 md:w-2/5">
          {/* <p className="text-3xl font-bold text-black">Token Distribution: </p> */}
          <p className="p-1 text-black">
            1. <span className="font-semibold">Scalability and Speed </span>: <br/> - Utilizes the proof-of-stake (PoS) consensus mechanism to enhance scalability, enabling higher transaction throughput and faster processing times. <br />
   - Implementation of shard chains, dividing the network into smaller chains to process transactions in parallel, significantly boosting network capacity. <br />
            2. <span className="font-semibold">Sustainability</span>: <br/> 40% -
            Allocated for funding charitable projects and initiatives that bring
            hope to communities in need. <br />
          
          </p>
        </div>
        <div className="bg-[#48E8ED] rounded-lg flex justify-start flex-col items-center p-4 md:w-2/5 shadow-md m-2">
          {/* <p className="text-3xl font-bold text-black">Token Utility: </p> */}
          {/* <p className="p-1 italic font-bold text-black">Bardoge is listed on Arbitrum Chain.</p> */}
          <p className="text-black">
          3. <span className="font-semibold">Security and Decentralization </span>: <br/> 
          - Enhances network security through PoS, incentivizing token holders to validate transactions and secure the network by staking their ETH. <br/>
   - Maintains decentralization by allowing a broader range of participants to engage in network validation and decision-making.
            <br />
            
          </p>
        </div>
      </div>

      <section class="py-12 px-4 text-center" id="team">
        <h2 class="text-3xl font-semibold mb-8">Meet Our Partners</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class=" rounded-lg shadow-md p-6 bg-[#48E8ED]">
            <a href="https://dx.app"> 
              <img
                src="dxsale.png"
                alt="Team Member 1"
                class="w-32 h-32 rounded-full mx-auto mb-4"
              />
            </a>
            <h3 class="text-lg font-semibold mb-2 text-black">DxSale</h3>
            {/* <p class="text-gray-600">Lead Designer</p> */}
          </div>

          <div class="bg-[#48E8ED] rounded-lg shadow-md p-6 ">
            <img
              src="https://th.bing.com/th/id/OIP.1wTTp6-XPR-g6PS8D8ZAqAHaDt?pid=ImgDet&rs=1"
              alt="Team Member 2"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-black">BNB Chain</h3>
            {/* <p class="text-gray-600">Senior Developer</p> */}
          </div>

          <div class="bg-[#48E8ED] rounded-lg shadow-md p-6">
            <img
              src="https://th.bing.com/th/id/OIP.mleb5_jAlRxR3Yj8Zksc2gHaHa?pid=ImgDet&rs=1"
              alt="Team Member 3"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-black">Chain Link</h3>
            {/* <p class="text-gray-600">Marketing Manager</p> */}
          </div>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 my-4">
          <div class="bg-[#48E8ED] rounded-lg shadow-md p-6">
            <img
              src="https://pbs.twimg.com/profile_images/1653532864309239810/ZjT_zBAS_400x400.png"
              alt="Team Member 1"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-black">ARB</h3>
            {/* <p class="text-gray-600">Lead Designer</p> */}
          </div>

          <div class="bg-[#48E8ED] rounded-lg shadow-md p-6">
            <img
              src="data:image/jpeg;base64,"
              alt="Team Member 2"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-black">Coinbase</h3>
            {/* <p class="text-gray-600">Senior Developer</p> */}
          </div>

          <div class="bg-[#48E8ED] rounded-lg shadow-md p-6">
            <img
              src="https://th.bing.com/th/id/R.95bf06d513ed7ae2ea7ee2457a8daf91?rik=IfsTHJ7lZxWswQ&pid=ImgRaw&r=0"
              alt="Team Member 3"
              class="w-32 h-32 rounded-full mx-auto mb-4"
            />
            <h3 class="text-lg font-semibold mb-2 text-black">1inch</h3>
            {/* <p class="text-gray-600">Marketing Manager</p> */}
          </div>
        </div>
      </section>

      <div
        id="footer"
        className="p-4 py-10 pb-4 bg-[#48E8ED] flex justify-center flex-col items-center"
      >
        <p className="text-3xl font-bold text-black">Social Accounts</p>
        <div className="flex justify-between mt-6">
          <a
            className="m-2"
            href=""
          >
            <FontAwesomeIcon
              icon={faXTwitter}
              size="lg"
              className="w-8 h-8 text-black hover:text-[#24038B]"
            />
          </a>
          <a className="m-2" href="">
            <FontAwesomeIcon
              icon={faTelegram}
              size="lg"
              className="w-8 h-8 text-black hover:text-[#24038B]"
            />
          </a>
        </div>
      </div>

      <div
        id="endbar"
        class="flex justify-center bg-[#48E8ED] py-1 px-4 items-center"
      >
        <p className="font-bold text-black">Listed on Binance Smart Chain</p>
      </div>
    </>
  );
}
